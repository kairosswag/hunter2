...
impl DateMatcher {
    pub fn new() -> DateMatcher {
        DateMatcher {
            ...
            maybe_date_sep : Regex::new(r"^(?P<g1>(\d{1,2})|\d{4})[\.,;\-/](?P<g2>\d{1,2})[\.,;\-/](?P<g3>(\d{1,2})|\d{4})$").unwrap(),
        }
    }
    ...
    fn valid_no_seperator(elem : &str) -> bool {
        ...
        return date_reg.iter().any(|dr| dr.is_match(elem));
    }
    ...
    fn match_pwd(&self, pwd : &str) -> MatchResult {
        ...
        for start_pos in 0 .. len - 4 {
            for window_size in 4 .. 9 {
                if start_pos + window_size > len { break };
        ...
        MatchResult { matcher_name : "DateMatcher".to_string(), matches : matches}
    }
}
...
mod tests {
    ...
    fn DateRegexTest() {
        ...
        let count = shall_not_pass.iter().filter(|nondate| maybe_date_sep.is_match(nondate)).count();
        assert_eq!(0, count);
    }
